...
import com.cxb.cxb_object.App_HeadersPropertyUtils;
...
public class HttpUtils {
    ...
    static {
        try {
            //User-Agent
            String USER_AGENT = App_HeadersPropertyUtils.getUserAgent();
...
//            }, 1, 5, TimeUnit.SECONDS);
    ...
    public static String doGet(URI uri, Map<String, String> headers
        ...
        try {
            ...
            result = getResponse(getHttpGet(uriBuilder.build(), headers), networkHeaders);
        } catch (Exception e) {
            AppAssert.fail(e);
        } finally {
            //不可以关闭，不然连接池就会被关闭
//            httpClient.close();
    ...
    private static String getResponse(HttpRequestBase httpRequestBase
        ...
        try {
            ...
            if (closeableHttpResponse.getStatusLine().getStatusCode() != 200) {
                result = errCodeFormat(closeableHttpResponse.getStatusLine().getStatusCode());
            } else {
        ...
        return getURLDecoderString(result, StandardCharsets.UTF_8);
    ...
    private static String toHttpGetParams(Map<String, String> param) throws Exception {
        ...
        for (Map.Entry<String, String> entry : param.entrySet()) {
            res += entry.getKey() + "=" + URLEncoder.encode(entry.getValue(), StandardCharsets.UTF_8.name()) + "&";
        }
     ...
     * @return
     */
    public static String errCodeFormat(int errCode) {
